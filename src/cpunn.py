...
def train_model(rank):
    ...
    df = pd.read_csv(file_name.format(rank+8))
    input_d = int(len(df.columns)-2)
    net = Net(y=input_d)

    target = df.iloc[:, 1]
    max_value = target.max()
    target = target.div(max_value)
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=int(800 / 12), shuffle=True)
...
if __name__ == '__main__':
    ...
    for rank in range(num_processes):
        p = multiprocessing.Process(target=train_model, args=(rank,))
        p.start()
        processes.append(p)
